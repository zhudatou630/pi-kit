...
找到一条能反映任务状态的只读命令。约定：退出码 0 = 正常，非 0 = 需要关注。

注意：任务完成也算「需要关注」——完成后续要 agent 收尾（停机、核验产物、通知用户），不能静默。所以检查命令对「完成」要返回非 0，否则会静默漏报。退出码管是否唤醒，stdout 内容管事件类型（完成 / 崩溃 / 不可达），agent 被唤醒后读证据判断。例如：
...
- **本任务动作授权**：完成 / 崩溃 / 不可达 各怎么处理。给出默认（见第4步 md 模板），用户确认或微调。
- **监工 session 的模型和 effort**：`provider` / `modelId` / `thinkingLevel` 用什么。附推荐（如「建议跟主 session 一致、effort 中等」），用户定。监工 session 长期复用，选错影响每次唤醒，值得配置时确认。
...
`timeout` 可选，默认 120 秒。检查命令超时后视为异常（退出码 124）。`provider` / `modelId` / `thinkingLevel` 在第5步创建 session 时用，pi-watch 不读它们。

同时写一份 `.pi/watch/<task>.md`，作为**监工 session 被唤醒时的唯一依据**——监工 session 不加载本 skill、不读 SKILL.md，只读这份 md + 项目 AGENTS.md + handoff。所以 md 必须自包含、写清晰：动作授权不能含糊，操作指路要指向真实存在的路径，任务描述要带 run_id 和预期产物。md 写漏或含糊，监工 session 就只能靠猜或事事问用户。

md 模板（用占位符，按项目实际填写；不要写死某个项目的具体路径）：
...
- 凭据：<凭据路径>
- 停机：<停机方式 / 脚本路径>
- resume：<resume 命令位置，如 handoff 训练启动部分>
```

默认授权针对**远端长任务**（按时计费实例）场景。纯本地长任务（本地编译、本地回测）请调整授权（如完成 → 只通知、不停机）。

### 5. 创建 session

创建或复用 Pi-Web session（名称如「监工｜项目名」），创建时带 config 里的 `provider` / `modelId` / `thinkingLevel`。具体调用方式见 `references/pi-web-wakeup.md`。每个项目维护一个稳定 session，多次任务用 run ID 区分。
...
pi-watch 唤醒消息带退出码、证据路径，并指路「先读 `.pi/watch/<task>.md` 和项目 AGENTS.md」。监工 session 不加载本 skill——它的动作依据是 `.pi/watch/<task>.md`（任务具体授权，优先于下面的通用原则），不是本节。本节面向配置者和文档读者，说明通用兜底原则：
